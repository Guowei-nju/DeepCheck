...
echo "Base pair count is: " $charcount
...
# echo "Splitting genome files into chunks of" $genoDiamondRunnerme_chunk "base-pairs..."
...
do
  ...
  reformat.sh in=$genomeName".faa" out=$genomeName"_"$samplerate"_pc_complete.faa" samplerate=`bc -l <<< "$samplerate / 100"` sampleseed=1 ignorejunk=t
  ...
  if (($samplerate >  30)); then
    ...
    int25pc=${f25pc%%.*}
    int30pc=${f30pc%%.*}
    reformat.sh in=cont_source.faa out=2pc.faa samplereadstarget=$int2pc ignorejunk=t
    reformat.sh in=cont_source.faa out=5pc.faa samplereadstarget=$int5pc ignorejunk=t
    reformat.sh in=cont_source.faa out=8pc.faa samplereadstarget=$int8pc ignorejunk=t
    reformat.sh in=cont_source.faa out=10pc.faa samplereadstarget=$int10pc ignorejunk=t
    reformat.sh in=cont_source.faa out=15pc.faa samplereadstarget=$int15pc ignorejunk=t
    reformat.sh in=cont_source.faa out=20pc.faa samplereadstarget=$int20pc ignorejunk=t
    reformat.sh in=cont_source.faa out=25pc.faa samplereadstarget=$int25pc ignorejunk=t
    reformat.sh in=cont_source.faa out=30pc.faa samplereadstarget=$int30pc ignorejunk=t
    rm cont_source.faa
...
for i in *.faa; do sed 's/\*//g' -i $i; done

mkdir /data/yan_code/deepcheck/data/traing_data/all_genomes
cp *.faa /data/yan_code/deepcheck/data/traing_data/all_genomes/
